...
try {
    ...
    # define VM
    Add-LabMachineDefinition -Name 'cvweb-latestbuild' -Memory 2GB -Network $defaultNetworkSwitch -Roles SQLServer2019 -OperatingSystem 'Windows 10 Enterprise Evaluation' 

    Install-Lab -Verbose

    # grant service logon privilege to the default user
    Invoke-LabCommand -ActivityName 'Grant Service-Logon Privilege to the default user' -ComputerName 'cvweb-latestbuild' -FilePath $labSources\CustomScripts\grant_service_logon_privilege.ps1 -Verbose
    
    #install jdk-8 - x64
    Install-LabSoftwarePackage -ComputerName 'cvweb-latestbuild' -Path $labSources\SoftwarePackages\jdk-8u321-windows-x64.exe -CommandLine '/s /log C:\DeployDebug\Java.log /Q' -Verbose

    Show-LabDeploymentSummary -Detailed

    Checkpoint-LabVM -ComputerName 'cvweb-latestbuild' -SnapshotName 'After creating a new VM with win10, jdk8 and SQLServer2019'
